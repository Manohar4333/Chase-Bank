...
app.post('/login', async (req, res) => {
  ...
  try {
    ...
    // Step 3: Redirect based on role
      ...
      return res.status(200).send({ message: 'Login successful', role: 'employee', employeeId: user._id });
    } else if (user.role === 'customer') {
      return res.status(200).send({ message: 'Login successful', role: 'customer',customerId:user._id });
    }
